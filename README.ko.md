...
| Claude 설정 | 전역 Claude 설정을 그대로 사용 | 기본 실행 시 `--setting-sources project,local`로 전역 플러그인 충돌 회피 |
...
MCP tool schema 오류가 나면 수동 `claudex run ...` 대신 기본 `claudex5` 명령을 사용하세요. 기본 경로는 전역 Claude 플러그인을 제외해 Firebase MCP 같은 외부 플러그인 스키마 문제를 피합니다.
